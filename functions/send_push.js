...
async function signJwt(header, claimSet, privateKeyPem) {
  ...
  const encodedSignature = base64url(bufferToString(new Uint8Array(signature)));
...
async function getFirebaseAccessToken() {
  ...
  const jwt = await signJwt(header, claimSet, serviceAccount.private_key);
...
module.exports = async function(request) {
  ...
  try {
    ...
    if (!fcm_token) {
        return new Response(JSON.stringify({ error: "No fcm_token provided" }), { status: 400, headers: corsHeaders });
    }
